...
def run_predictions(cols, model_uri):
    #for i in range(1,100):
        ...
        # Load the model
        with open('XGboost/experiment_1/model/trained_model_1.pkl', 'rb') as file:
            XGB_model = pickle.load(file)

        '''
        with open(f'bayesian_tests/experiment_15/model/no_calibration/trained_model_{i}.pkl', 'rb') as file:
            nc_model = pickle.load(file)
        with open(f'bayesian_tests/experiment_15/model/isotonic/trained_model_{i}.pkl', 'rb') as file:
            isotonic_model = pickle.load(file)
        with open(f'bayesian_tests/experiment_15/model/sigmoid/trained_model_{i}.pkl', 'rb') as file:
            sigmoid_model = pickle.load(file)
        '''
        # Predict probabilities using the calibrated model
        
        xgb_probs = XGB_model.predict(X)
        '''
        ...
        for idx, (_, row) in enumerate(data.iterrows()):
            ...
            home_odds = row['lowvig_home']
            away_odds = row['lowvig_away']
            ...
            xgb_home_win_prob = xgb_probs[idx, 1]
            xgb_away_win_prob = xgb_probs[idx, 0]
            '''
            ...
            xgb_preds.append([game_id, game_date, row['home_team'], row['away_team'], row['home_is_winner'], 
                                         home_odds, away_odds, home_implied_prob, away_implied_prob,
                                         xgb_home_win_prob, xgb_away_win_prob])
            '''
        ...
        for preds, name in zip(df_list, df_names):
            ...
            df.to_csv(f'bayesian_tests/experiment_15/predictions/{name}/test_predictions_{i}.csv', index=False)
        '''
        df = pd.DataFrame(xgb_preds, columns=[
                    'id', 'game_date', 'home_team', 'away_team', 'home_is_winner', 
                    'home_odds', 'away_odds', 'home_implied_prob', 'away_implied_prob',
                    'model_home_win_prob', 'model_away_win_prob'])

        df.to_csv('XGboost/experiment_1/predictions/test_predictions_1.csv')