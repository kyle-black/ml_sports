...
############################### Add Win Pct ##################################     
    def combine_game_win(self):
        ...
        self.merged_df = self.past_games.merge(self.home_win_pct, how='left', left_on=['home_team', 'game_date'], right_on = ['home_Team', 'home_Date'])
...
######################################COMBINE GAME OPP RUNS #########################################
    ...
    def update_columns(self):
        ...
        #return self.merged_df.to_csv('combined_runs.csv')
        
        '''
        self.merged_df= self.merged_df[['id', 'game_date', 'season',
       ...
       'away_opp_1', 'away_opp_Home', 'away_opp_Previous', 'lowvig_home', 'lowvig_away', 'bovada_home', 'bovada_away']]
        '''

        self.merged_df= self.merged_df[['id', 'game_date', 'season',
       'home_team', 'away_team', 'home_score', 'away_score', 'home_is_winner',
       'away_is_winner', 'game_type', 'away_Date','home_Current_win_pct','home_prev_win_pct','Rolling_10D_win_x','Rolling_30D_win_x', 'away_Current_win_pct', 'away_prev_win_pct', 
         'Rolling_10D_win_y', 'Rolling_30D_win_y', 'home_r_Current','home_r_prev', 'Rolling_10D_r_home', 
         'Rolling_30D_r_home', 'away_r_Current','away_r_prev','Rolling_10D_r_away','Rolling_30D_r_away', 'home_opp_Current',
         'home_opp_prev', 'Rolling_10D_opp_home', 'Rolling_30D_opp_home', 'away_opp_Current', 'away_opp_prev', 'Rolling_10D_opp_away', 'Rolling_30D_opp_away', 'lowvig_home', 'lowvig_away']]
        '''
        self.merged_df = self.merged_df = [['id', 'game_date', 'season',
       'home_team', 'away_team', 'home_score', 'away_score', 'home_is_winner',
       'away_is_winner', 'game_type', 'away_Date', 'lowvig_home', 'lowvig_away', 'pinnacle_home','pinnacle_away','betonlineag_home','betonlineag_away', 'unibet_home','unibet_away']]
#        self.merged_df= self.merged_df[['id', 'game_date', 'season', 'home_team', 'away_team','home_is_winner', 'lowvig_home', 'lowvig_away', 'bovada_home', 'bovada_away', 'betonlineag_home', 'betonlineag_away', 'unibet_home', 'unibet_away', 'sport888_home', 'sport888_away', 'draftkings_home','draftkings_away','pinnacle_home', 'pinnacle_away', 'mybookieag_home', 'mybookieag_away']]
        ...
        #self.merged_df = self.merged_df.drop_duplicates(subset='id', keep='last')
         ...
         # Calculate 'lowvig_home_vf' and 'lowvig_away_vf'
        
        # Calculate 'vig' for each row of the dataframe
        '''
        for col in self.merged_df.filter(like='_home', axis =1):
            self.merged_df[f'{col}_prob'] = self.merged_df[col].apply(self.american_to_implied_probability)


        for col in self.merged_df.filter(like='_away', axis =1):
            self.merged_df[f'{col}_prob'] = self.merged_df[col].apply(self.american_to_implied_probability)
       ...
       # book_list = ['lowvig','pinnacle','betonlineag_home','betonlineag_away', 'unibet_home','unibet_away'] 
...
###################################################################################################
        ...
        self.merged_df['pinnacle_home_prob'] = self.merged_df['pinnacle_home'].apply(self.american_to_implied_probability)
        self.merged_df['pinnacle_away_prob'] = self.merged_df['pinnacle_away'].apply(self.american_to_implied_probability)

        # Calculate vig
        self.merged_df['pinnacle_home_vig'] = self.merged_df.apply(self.calculate_vig, axis=1)
        self.merged_df['pinnacle_away_vig'] = self.merged_df.apply(self.calculate_vig, axis=1)

        # Calculate vig free probabilities
        total_prob = self.merged_df['pinnacle_home_prob'] + self.merged_df['pinnacle_away_prob']
        self.merged_df['pinnacle_home_vf'] = self.merged_df['pinnacle_home_prob'] / total_prob
        self.merged_df['pinnacle_away_vf'] = self.merged_df['pinnacle_away_prob'] / total_prob
        
        total_prob = self.merged_df['pinnacle_home_prob'] + self.merged_df['pinnacle_away_prob']
        self.merged_df['pinnacle_home_vf'] = self.merged_df['pinnacle_home_prob'] / total_prob
        self.merged_df['pinnacle_away_vf'] = self.merged_df['pinnacle_away_prob'] / total_prob
##########################################################################################################
        self.merged_df['betonlineag_home_prob'] = self.merged_df[book].apply(self.american_to_implied_probability)
        self.merged_df['betonlineag_home_vig'] = self.merged_df.apply(self.calculate_vig, axis=1)
        

        # Calculate vig free probabilities
        total_prob = self.merged_df['betonlineag_home_prob'] + self.merged_df['betonlineag_away_prob']
        self.merged_df['betonlineag_home_vf'] = self.merged_df['betonlineag_home_prob'] / total_prob
        self.merged_df['betonlineag_away_vf'] = self.merged_df['betonlineag_away_prob'] / total_prob

        self.merged_df['unibet_home_prob'] = self.merged_df[b].apply(self.american_to_implied_probability)
        self.merged_df['betonlineag_home_vig'] = self.merged_df.apply(self.calculate_vig, axis=1)
...
class Train_Model(Combine_Data):
    ...
    def prepare_data(self):
        ...
        self.X = self.merged_df[[ 'home_Current_win_pct','home_prev_win_pct','Rolling_10D_win_x','Rolling_30D_win_x', 'away_Current_win_pct', 'away_prev_win_pct', 
         'Rolling_10D_win_y', 'Rolling_30D_win_y', 'home_r_Current','home_r_prev', 'Rolling_10D_r_home', 
         'Rolling_30D_r_home', 'away_r_Current','away_r_prev','Rolling_10D_r_away','Rolling_30D_r_away', 'home_opp_Current',
         'home_opp_prev', 'Rolling_10D_opp_home', 'Rolling_30D_opp_home', 'away_opp_Current', 'away_opp_prev', 'Rolling_10D_opp_away', 'Rolling_30D_opp_away', 'lowvig_home_vf', 'lowvig_away_vf']]
        
        self.train_cols = self.X.columns.to_list()
       # return self.X['home_opp_Home']
    ...
    def run_training(self):
        ...
        #bayesian_test(self.train_cols,self.X,self.y,self.merged_df)
        df = log_training(self.X, self.y)
